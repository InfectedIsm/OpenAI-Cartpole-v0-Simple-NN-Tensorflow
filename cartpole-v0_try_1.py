...
def one_hotc_decoding(vector,dimension,min_val=0):    
...
#        	alpha = 0.03
        ...
        for _ in range(50):
            env.render()
            time.sleep(0.1) 
            action=sess.run(y, feed_dict={x: input_step})
...
#        print("episode simulation time : {:.2f} ms, dead at {} steps ".format((time.time()-t_episode)*1e3, t+1))
    ...
    x_test=test_set[:,0:5]
    y_test=test_set[:,5:6]
    graph=define_train_play_model(hidden_l_neurons=50, num_features=5, num_labels=1,
                                 x_train=x_train, y_train=y_train,
                                 alpha=0.3, number_of_training=20000, batch_size=100,
